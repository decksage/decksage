...
export default function DecklistVisualizer({ decklist, cardDataMap }: DecklistVisualizerProps) {
  ...
  const groupedCards = useMemo(() => {
    ...
    Object.values(groups).forEach((group) =>
      group.sort((a, b) => a.card.cmc - b.card.cmc || a.card.name.localeCompare(b.card.name)),
    );
    return groups;
  }, [allCardsWithData]);
  ...
  const CardRow = ({ card, count }: { card: ScryfallCard; count: number }) => (
    <li
      key={card.id}
      className="flex justify-between items-center text-sm p-1.5 rounded-md cursor-pointer hover:bg-neutral-800 transition-colors"
    ...
    <Card className="bg-neutral-900 border-neutral-800 flex flex-col lg:flex-row">
      ...
      <div className="w-full lg:w-1/3 p-4 flex justify-center items-start lg:sticky lg:top-24 self-start border-b lg:border-b-0 lg:border-r border-neutral-800">
        {selectedCard ? (
          ...
          <div className="aspect-[5/7] w-full max-w-[300px] bg-neutral-950 rounded-xl flex items-center justify-center text-neutral-500">
            <p>Passe o mouse sobre uma carta</p>
          </div>
        )}
      </div>
                ...
                {TOP_ROW_ORDER.map((type) => {
                  ...
                  return (
                    <div key={type} className="mb-6">
                      <h4 className="font-semibold text-amber-400 mb-2">
                        {TYPE_DISPLAY_NAMES[type]} ({items.reduce((s, i) => s + i.count, 0)})
                      </h4>
                      <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-2">
                        {items.map(({ card, count }) => (
                          ...
                          >
                            ...
                            />
                            <div className="absolute top-1 right-1 bg-black/80 ...">{count}</div>
                          </div>
                        ))}
                      </div>
                    </div>
