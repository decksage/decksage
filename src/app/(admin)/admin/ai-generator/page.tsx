...
import {
  generateDeckContentPackage,
  saveAiGeneratedDeckAndContent,
} from '@/app/actions/admin/aiContentGeneratorActions';
import {
  Loader2,
  Sparkles,
  Wand2,
...
function GenerateButton({
  ...
  return (
    <Button size="lg" type="submit" className="w-full" disabled={isDisabled}>
      {pending ? (
        <>
          <Loader2 className="mr-2 h-5 w-5 animate-spin" /> Gerando conteúdo, isso pode levar um
          minuto...
        </>
      ) : (
        <>
  ...
  return (
    <Button type="submit" size="lg" disabled={pending} className="bg-green-600 hover:bg-green-700">
      {pending ? (
        ...
        <>
          <Save className="mr-2 h-4 w-4" /> Publicar Deck e Conteúdo
        </>
      )}
    </Button>
  );
      ...
      <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in">
        <header className="mb-2 text-center">
          <h1 className="text-3xl font-bold text-amber-400">
            {contentPackage.name || 'Nome não gerado'}
          </h1>
          <p className="text-neutral-300 mt-2">
            {contentPackage.description || 'Descrição não gerada'}
          </p>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="bg-neutral-900 border-neutral-800">
            ...
            <CardContent>
              <pre className="text-sm bg-neutral-950 p-3 rounded-md max-h-96 overflow-auto">
                {contentPackage.decklist?.mainboard
          ...
          <Card className="bg-neutral-900 border-neutral-800">
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <BookOpenText size={20} /> Deck Check
              </CardTitle>
            </CardHeader>
            <CardContent className="text-sm space-y-2">
              <p>
                <strong>Estilo:</strong> {contentPackage.deck_check?.playstyle ?? 'N/A'}
              </p>
              <p>
                <strong>Vitória:</strong> {contentPackage.deck_check?.win_condition ?? 'N/A'}
              </p>
              <p>
                <strong>Dificuldade:</strong> {contentPackage.deck_check?.difficulty ?? 'N/A'}
              </p>
              <p>
                <strong>Pontos Fortes:</strong>{' '}
                {contentPackage.deck_check?.strengths?.join(', ') ?? 'N/A'}
              </p>
              <p>
                <strong>Pontos Fracos:</strong>{' '}
                {contentPackage.deck_check?.weaknesses?.join(', ') ?? 'N/A'}
              </p>
            </CardContent>
          </Card>
        </div>
        <Card className="bg-neutral-900 border-neutral-800">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Newspaper size={20} /> Guia de Como Jogar
            </CardTitle>
          </CardHeader>
          <CardContent className="text-sm p-4 bg-neutral-950 rounded-md whitespace-pre-wrap leading-relaxed">
            {contentPackage.how_to_play_guide ?? 'Não gerado.'}
          </CardContent>
        </Card>
        <Card className="bg-neutral-900 border-neutral-800">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <MessageSquare size={20} /> Posts para Redes Sociais
            </CardTitle>
          </CardHeader>
          <CardContent>
            <Tabs defaultValue="facebook">
              ...
              <TabsContent
                value="facebook"
                className="text-sm p-4 bg-neutral-950 rounded-b-md rounded-r-md whitespace-pre-wrap"
              >
                {contentPackage.social_posts?.facebook ?? 'Não gerado.'}
              </TabsContent>
              <TabsContent
                value="instagram"
                className="text-sm p-4 bg-neutral-950 rounded-b-md rounded-r-md whitespace-pre-wrap"
              >
                {contentPackage.social_posts?.instagram ?? 'Não gerado.'}
              </TabsContent>
              <TabsContent
                value="x"
                className="text-sm p-4 bg-neutral-950 rounded-b-md rounded-r-md whitespace-pre-wrap"
              >
                {contentPackage.social_posts?.x ?? 'Não gerado.'}
              </TabsContent>
              <TabsContent
                value="reddit"
                className="text-sm p-4 bg-neutral-950 rounded-b-md rounded-r-md whitespace-pre-wrap"
              >
                {contentPackage.social_posts?.reddit ?? 'Não gerado.'}
              </TabsContent>
            </Tabs>
          </CardContent>
        </Card>
        ...
        <form action={saveAiGeneratedDeckAndContent} className="flex justify-end gap-4 pt-4">
          ...
          <input
            type="hidden"
            name="decklist"
            value={JSON.stringify(contentPackage.decklist ?? {})}
          />
          <input
            type="hidden"
            name="deck_check"
            value={JSON.stringify(contentPackage.deck_check ?? {})}
          />
          <input type="hidden" name="how_to_play_guide" value={contentPackage.how_to_play_guide} />
          <input
            type="hidden"
            name="social_posts"
            value={JSON.stringify(contentPackage.social_posts ?? {})}
          />
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <header className="mb-10 text-center">
        ...
        <h1 className="text-4xl font-extrabold text-amber-400 mt-4 mb-2">
          Super Gerador de Conteúdo
        </h1>
        <p className="text-lg text-neutral-300">
          Gere um deck, análise e posts para redes sociais com um único clique.
        </p>
      </header>
      <form action={formAction}>
        <Card className="bg-neutral-900 border-neutral-800">
          <CardHeader>
            ...
            <CardDescription>
              Insira uma ideia ou tema para o deck e selecione o formato.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-2">
              ...
              <Select
                ...
                value={format}
              >
                <SelectTrigger className="bg-neutral-800">
              ...
              <div className="space-y-2 animate-in fade-in">
                ...
                {commander && (
                  <>
                    ...
                    <input
                      type="hidden"
                      name="commanderColorIdentity"
                      value={JSON.stringify(commander.color_identity)}
                    />
                  </>
                )}
              </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                required
                minLength={20}
                placeholder="Ex: Um deck de Commander com 'Atraxa, Praetors' Voice' focado em Proliferate e marcadores +1/+1, com uma pegada de controle."
                rows={4}
              />
            </div>
          </CardContent>
