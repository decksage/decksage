...
function SubmitButton() {
  ...
  return (
    ...
    >
      {pending ? (
        <>
          <Loader2 className="mr-2 h-4 w-4 animate-spin" /> Salvando Deck...
        </>
      ) : (
        <>
          <Save className="mr-2 h-4 w-4" /> Criar e Salvar Deck
        </>
      )}
    </Button>
  );
    ...
    <div className="max-w-7xl mx-auto">
      <header className="mb-8">
        ...
        <p className="text-neutral-400 mt-1">
          Crie decks para serem exibidos como conteúdo da plataforma.
        </p>
      </header>
      <form
        onSubmit={async (e) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
              ...
              <CardContent className="space-y-4">
                ...
                {format === 'commander' && (
                  <div className="space-y-1">
                    ...
                    <AutocompleteInput onSelect={setCommander} placeholder="Buscar comandante..." />
                  </div>
                )}
                <div className="space-y-1">
                  ...
                  <Textarea
                    ...
                    placeholder="Estratégia, tema, etc."
                  />
                </div>
              </CardContent>
            </Card>
            ...
            <Tabs defaultValue="mainboard">
              <TabsList>
                <TabsTrigger value="mainboard">Deck Principal</TabsTrigger>
              ...
              <TabsContent value="mainboard">
                <DecklistEditor
                  ...
                  cards={mainboard
                    .map((c) => ({ count: c.count, card: cardDataMap.get(c.name)! }))
                    .filter((c) => c.card)}
                  onQuantityChange={(name, count) => handleQuantityChange(name, count, 'mainboard')}
                  onRemoveCard={(name) => removeCard(name, 'mainboard')}
                />
              </TabsContent>
              <TabsContent value="sideboard">
                <DecklistEditor
                  ...
                  cards={sideboard
                    .map((c) => ({ count: c.count, card: cardDataMap.get(c.name)! }))
                    .filter((c) => c.card)}
                  onQuantityChange={(name, count) => handleQuantityChange(name, count, 'sideboard')}
                  onRemoveCard={(name) => removeCard(name, 'sideboard')}
                />
              </TabsContent>
            </Tabs>
          </div>
          ...
          <aside className="lg:col-span-1 space-y-6 lg:sticky lg:top-8">
            <Card className="bg-neutral-900 border-neutral-800">
              ...
              <CardContent className="space-y-4">
                <div className="space-y-1">
                  <Label>Adicionar ao Deck</Label>
                  <AutocompleteInput onSelect={(card) => addCard(card, 'mainboard')} />
                </div>
                <div className="space-y-1">
                  <Label>Adicionar ao Sideboard</Label>
                  <AutocompleteInput onSelect={(card) => addCard(card, 'sideboard')} />
                </div>
              </CardContent>
            </Card>
            <SubmitButton />
