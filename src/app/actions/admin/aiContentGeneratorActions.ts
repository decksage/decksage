 ...
 */
export async function generateDeckContentPackage(
  prevState: any,
  formData: FormData,
  ...
  try {
    ...
    const deckPrompt = createDecklistPrompt({
      format,
      userPrompt,
      commanderName,
      commanderColorIdentity,
    });
    const deckCompletion = await openai.chat.completions.create({
    ...
    if (deckResult.decklist.sideboard)
      deckResult.decklist.sideboard = parseDecklistArray(deckResult.decklist.sideboard);
    ...
    const decklistString = deckResult.decklist.mainboard
      .map((c: CoreCard) => `${c.count} ${c.name}`)
      .join('\n');
    ...
    const deckCheckCompletion = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
    ...
    const howToPlayPrompt = createHowToPlayGuidePrompt({
      deckName: deckResult.name,
      deckDescription: deckResult.description,
      deckCheck: deckCheckResult,
    });
    const howToPlayCompletion = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [{ role: 'user', content: howToPlayPrompt }],
    });
    const howToPlayGuide =
      howToPlayCompletion.choices[0].message.content || 'Não foi possível gerar o guia.';
    ...
    const { data: logEntry } = await supabase
      ...
      .insert({
        ...
        generated_deck_name: contentPackage.name,
        deck_check: contentPackage.deck_check,
        social_posts: contentPackage.social_posts,
        how_to_play_guide: contentPackage.how_to_play_guide,
      })
      .select('id')
      .single();
    ...
    return {
      success: false,
      error: 'Falha ao gerar o pacote de conteúdo. Verifique o console do servidor.',
    };
  }
}
...
export async function saveAiGeneratedDeckAndContent(formData: FormData) {
  ...
  try {
    ...
    const representative_card_image_url = scryfallData.find(
      (c) => c.name === (commanderName || allCardNames[0]),
    )?.image_uris?.art_crop;
