...
export default async function AdminPage() {
  ...
  const [{ count: userCount }, { count: deckCount }, { data: usersByDayData }, analyticsData] =
    await Promise.all([
  ...
  const chartData = (usersByDayData || [])
    .map((row: any) => ({
      date: new Intl.DateTimeFormat('pt-BR', { month: 'short', day: 'numeric' }).format(
        new Date(row.day),
      ),
      total: row.count,
    }))
  ...
  return (
    <>
      <header className="mb-10">
        <h1 className="text-4xl md:text-5xl font-bold text-amber-500 tracking-tight">Dashboard</h1>
        <p className="text-lg text-neutral-400 mt-2">Visão geral e métricas da plataforma.</p>
      </header>
      ...
      {analyticsData.error && (
        <div className="mb-6 p-4 bg-red-900/20 border border-red-500/30 rounded-lg text-red-300">
          <p>
            <strong>Erro ao carregar dados do Analytics:</strong> {analyticsData.error}
          </p>
          <p className="text-sm mt-2">
            Verifique se suas variáveis de ambiente e permissões no Google estão corretas.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <StatCard
          ...
          value={deckCount ?? 0}
          description="Decks criados na plataforma"
          Icon={FileText}
        />
        ...
        <StatCard
          ...
          description="Visitantes nos últimos 7 dias"
          Icon={Users}
        />
        <StatCard
          title="Page Views (7d)"
