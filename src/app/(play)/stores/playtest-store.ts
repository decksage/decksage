...
export const usePlaytestStore = create<PlaytestState>((set, get) => ({
  ...
  actions: {
    initializeDeck: (decklist, commanderList, scryfallMap, deckFormat) => {
      ...
      if (
        (!potentialCommanders || potentialCommanders.length === 0) &&
      ...
      const fullLibrary = decklist
        ...
        .flatMap((item) => {
          ...
          return Array.from({ length: item.count }, (_, i) =>
            createCardInstance(cardData, `${cardData.id}-${i}`),
          );
        });
    ...
    moveCardToLibrary: (cardInstanceId, fromZone, position) => {
      set((state) => {
        ...
        toast.info(
          `${cardToMove.name} foi para o ${position === 'top' ? 'topo' : 'fundo'} do grimório.`,
        );
    ...
    returnCommanderToZone: (cardInstanceId: string) => {
      set((state) => {
        ...
        toast.error(
          `${cardToMove.name} voltou para a Zona de Comando. Imposto: +${cardToMove.commanderTax}`,
        );
