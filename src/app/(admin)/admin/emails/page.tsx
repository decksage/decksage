...
export default function EmailBroadcastPage() {
  ...
  const initiateSend = (emailData?: any) => {
    ...
    if (isTest && !testEmail && !emailData) {
      // If editor is test mode (list mode is always production/send now, or we should clarify)
  ...
  const executeSend = async (data: any) => {
    startTransition(async () => {
      ...
      if (data.blocks)
        formData.append(
          'blocks',
          typeof data.blocks === 'string' ? data.blocks : JSON.stringify(data.blocks),
        );
            ...
            <Card className="bg-neutral-900 border-neutral-800">
              ...
              <CardContent className="space-y-6">
                <div className="flex items-center space-x-2">
                  <Switch id="mode-toggle" checked={isTest} onCheckedChange={setIsTest} />
                  ...
                  >
                    {isPending ? (
                      ...
                      <>
                        ...
                        {isTest ? 'Enviar Teste' : 'PUBLICAR E ENVIAR'}
                      </>
                    )}
                  </Button>
                </div>
        ...
        <AlertDialogContent className="bg-neutral-900 border-neutral-800 text-neutral-200">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-neutral-400 text-base">
              Você está prestes a enviar o email <strong>"{pendingSend?.subject}"</strong> para{' '}
              <strong>TODOS</strong> os usuários da base.
              <br />
              <br />
