...
export async function sendBroadcastEmail(prevState: any, formData: FormData) {
    ...
    if (!isAdmin) {
        return { success: false, message: 'Acesso negado. Apenas administradores podem enviar emails.' };
    }
    ...
    try {
        if (isTest) {
            ...
            return { success: true, message: `Disparo concluído! Email enviado para ${sentCount} usuários.` };
        }
    } catch (error: any) {
...
export async function saveDraft(formData: FormData) {
    ...
    if (!await checkUserRole('admin')) {
        return { success: false, message: "Acesso negado. Apenas administradores podem salvar rascunhos." };
    }
    ...
    try {
        ...
        if (id) {
            const { error } = await supabase.from('email_broadcasts')
                .update({
                    subject,
                    content,
                    blocks,
            ...
            const { data, error } = await supabase.from('email_broadcasts')
                .insert({
                    admin_id: user?.id,
                    subject,
                    content,
