...
export async function createAdSlot(prevState: FormState, formData: FormData): Promise<FormState> {
  ...
  if (!slot_name || slot_name.trim().length < 3) {
    return {
      message: 'O nome do slot é obrigatório e precisa ter pelo menos 3 caracteres.',
      success: false,
    };
  }
...
export async function updateAdSlot(
  ...
  const { error } = await supabase.from('ad_slots').update(updates).eq('id', slotId);
