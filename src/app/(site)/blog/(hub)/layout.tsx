...
async function BlogSidebar() {
  ...
  const popularPostsPromise = supabase
    .from('posts')
    .select('title, slug')
    .eq('status', 'published')
  ...
  return (
    <aside className="w-full lg:w-1/4 lg:pl-8 space-y-8">
      ...
      <div className="p-4 bg-neutral-900 rounded-lg border border-neutral-800">
        <h3 className="font-bold text-lg text-amber-500 mb-2 flex items-center gap-2">
        ...
        <input
          ...
          className="w-full p-2 rounded bg-neutral-800 border-neutral-700 focus:ring-amber-500 focus:border-amber-500"
        />
      </div>
          ...
          {categories?.map((cat) => (
            <li key={cat.slug}>
              <Link
                ...
                className="text-neutral-300 hover:text-amber-400 transition-colors block p-1 rounded"
              >
                {cat.name}
              </Link>
            </li>
