              ...
              {feedback.comments?.map((comment: any) => (
                <div key={comment.id} className="flex items-start gap-3">
                  <Avatar className="h-9 w-9 mt-1">
                    ...
                    <AvatarFallback>
                      {comment.admin_username?.charAt(0).toUpperCase()}
                    </AvatarFallback>
                  </Avatar>
          ...
          <Card className="bg-neutral-900 border-neutral-800">
            ...
            <CardContent className="space-y-3 text-sm text-neutral-400">
              <div className="flex items-center gap-2">
                <User size={16} /> <strong>Usuário:</strong>{' '}
                {feedback.submitter_profile?.username
                  ? `@${feedback.submitter_profile.username}`
                  : feedback.submitter_profile?.email || 'Anônimo'}
              </div>
              <div className="flex items-center gap-2">
                <Clock size={16} /> <strong>Enviado em:</strong>{' '}
              ...
              <div className="flex items-start gap-2">
                <Link
                  ...
                  target="_blank"
                  className="truncate hover:text-amber-400"
                >
                  <strong>URL:</strong> {feedback.page_url || 'Não informada'}
                </Link>
              </div>
              <div className="flex items-start gap-2">
                <Monitor size={16} />{' '}
                <span className="truncate" title={feedback.user_agent || ''}>
                  <strong>Agente:</strong> {feedback.user_agent || 'Não informado'}
                </span>
              </div>
            </CardContent>
          </Card>
