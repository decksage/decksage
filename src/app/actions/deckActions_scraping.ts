...
export async function createDeck(prevState: FormState, formData: FormData): Promise<FormState> {
  ...
  for (const cardName of allCardNames) {
    ...
    const { data: existingPrice } = await supabase
      ...
      .gte('last_updated_at', twentyFourHoursAgo)
      .single();

    if (!existingPrice) {
      console.log(
        `Preço para "${cardName}" não encontrado ou desatualizado. Buscando na Ligamagic...`,
      );
...
export async function updateDeckContent(
  ...
  const decklist: Decklist = {
    mainboard: parsedCards
      .filter((c) => !c.is_sideboard)
      .map(({ name, count, have_physical }) => ({ name, count, have_physical: !!have_physical })),
    sideboard: parsedCards
      .filter((c) => c.is_sideboard)
      .map(({ name, count, have_physical }) => ({ name, count, have_physical: !!have_physical })),
  };
