    ...
    <Card className="bg-neutral-900 border-neutral-800 h-full">
      ...
      <CardContent className="space-y-2 max-h-[500px] overflow-y-auto pr-3">
        {cards.map(({ card, count }) => (
          <div
            key={card.id}
            className="flex items-center justify-between p-2 rounded-md hover:bg-neutral-800/50"
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                size="icon"
                className="h-6 w-6"
                onClick={() => onQuantityChange(card.name, count - 1)}
              >
                <Minus size={14} />
              </Button>
        ...
        {cards.length === 0 && (
          <p className="text-center text-neutral-500 py-4">
            Adicione cartas ao {title.toLowerCase()}.
          </p>
        )}
      </CardContent>
    </Card>
