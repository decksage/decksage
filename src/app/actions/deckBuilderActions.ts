...
export async function buildDeckWithAI(
  ...
  try {
    ...
    return { success: false, error: 'Erro ao processar a lista de cartas.' };
  }
  ...
  try {
    const completion = await openai.chat.completions.create({
      messages: [
        { role: 'system', content: 'Aja como um especialista em Magic: The Gathering.' },
    ...
    if (
      !parsedResult.name ||
      !parsedResult.decklist ||
      !Array.isArray(parsedResult.decklist.mainboard)
    )
      return { success: false, error: 'A IA retornou uma estrutura de deck inválida.' };
    ...
    if (parsedResult.decklist.mainboard.length === 0)
      return { success: false, error: 'A IA não conseguiu gerar uma lista de cartas válida.' };

    const manaCurve = await calculateManaCurve(parsedResult.decklist);
    const cardCount =
      (parsedResult.decklist.mainboard.reduce((s: any, c: any) => s + c.count, 0) || 0) +
      (parsedResult.decklist.sideboard?.reduce((s: any, c: any) => s + c.count, 0) || 0);
    ...
    const { data: logEntry } = await supabase
      ...
      .insert({
        ...
        input_cards: cards,
        generated_deck_name: parsedResult.name,
