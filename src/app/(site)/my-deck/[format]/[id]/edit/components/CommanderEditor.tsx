...
export default function CommanderEditor({
  ...
  const handleCommanderChange = (newCommander: ScryfallCard) => {
    if (
      !newCommander.type_line?.includes('Legendary') ||
      !newCommander.type_line?.includes('Creature')
    ) {
    ...
    setCards((prevCards) => {
      ...
      if (isAlreadyInList) {
        toast.error(
          `${newCommander.name} já está no deck. Remova-o primeiro para defini-lo como comandante.`,
        );
        return prevCards;
      }
    ...
    <Card className="bg-neutral-900 border-neutral-800">
      ...
      <CardContent>
        {commanderCard ? (
          ...
          >
            ...
            <div className="flex items-center gap-2 p-1 rounded-md transition-colors bg-green-500/10 border border-green-500/20">
              ...
              <Button
                ...
                size="icon"
                className="h-6 w-6"
                onClick={() => handleCommanderCollectionUpdate(ownedCount - 1)}
          ...
          <AutocompleteInput
            onSelect={(card) => card && handleCommanderChange(card)}
            placeholder="Buscar novo comandante..."
          />
        )}
      </CardContent>
    </Card>
