...
const BattlefieldSubZone = ({
  ...
  return (
    <div
      className={cn(
        'relative bg-neutral-900/30 border border-neutral-700/50 rounded-md p-2',
        heightClass,
      )}
    >
...
export default function PlaytestView({
  ...
  const [zoneViewer, setZoneViewer] = useState<{ title: string; cards: GameCard[] }>({
    title: '',
    cards: [],
  });
  ...
  const { setNodeRef: handRef, isOver: isOverHand } = useDroppable({
    id: 'hand',
    data: { zone: 'hand' },
  });
  const { setNodeRef: commandZoneRef, isOver: isOverCommandZone } = useDroppable({
    id: 'commandZone',
  ...
  return (
    <DndContext
      sensors={sensors}
      onDragStart={(event) => setActiveCard(event.active.data.current?.cardObject)}
    ...
    >
        ...
        <main className="flex-1 flex flex-col p-4 gap-2">
          ...
          <div className="flex gap-4 h-[220px]">
            <div
              ref={handRef}
              className={cn(
                'flex-grow bg-black/20 rounded-lg p-4 border transition-colors',
                isOverHand ? 'border-amber-400' : 'border-neutral-800',
              )}
            >
              <h2 className="font-bold mb-2 text-sm text-neutral-300">
                Mão: {playtestState.hand.length} cartas
              </h2>
            ...
            {deckFormat === 'commander' && (
              <div
                ref={commandZoneRef}
                className={cn(
                  'w-44 flex-shrink-0 bg-black/20 rounded-lg p-2 border transition-colors',
                  isOverCommandZone ? 'border-amber-400' : 'border-neutral-800',
                )}
              >
                <h2 className="font-bold text-xs text-center text-neutral-300 mb-1 flex items-center justify-center gap-1">
                  <Crown size={14} /> Comando
                </h2>
      ...
      <DragOverlay>
        {activeCard ? (
          <div className="w-28 opacity-90 rotate-3 shadow-2xl">
            <Image
              src={activeCard.image_uris?.small || ''}
              alt={activeCard.name}
              width={146}
              height={204}
              unoptimized
      ...
      <ZoneViewerModal
        isOpen={!!zoneViewer.title}
        onOpenChange={(isOpen) => !isOpen && setZoneViewer({ title: '', cards: [] })}
        zoneName={zoneViewer.title}
        cards={zoneViewer.cards}
      />
      <CardDetailModal
        isOpen={!!detailModalCard}
        onOpenChange={(isOpen) => {
          if (!isOpen) setDetailModalCard(null);
        }}
