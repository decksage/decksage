        ...
        <ContextMenuContent className="w-60 bg-neutral-900 border-neutral-700 text-neutral-200">
          ...
          {(zoneId === 'graveyard' || zoneId === 'exile') && topCard && (
            <>
              ...
              <ContextMenuItem
                onSelect={() => actions.moveCard(topCard.instanceId, zoneId, 'hand')}
                className="cursor-pointer"
              >
                <Hand className="mr-2 h-4 w-4" /> Mover para a Mão
              </ContextMenuItem>
              <ContextMenuItem
                onSelect={() => actions.moveCard(topCard.instanceId, zoneId, 'battlefield')}
                className="cursor-pointer"
              >
          ...
          {zoneId === 'graveyard' && count > 0 && (
            <ContextMenuItem
              onSelect={() => actions.shuffleGraveyardIntoLibrary()}
              className="cursor-pointer"
            >
...
export default function PlaytestSidebar({
  deckFormat,
  onViewZone,
}: {
  deckFormat: string;
    ...
    <aside className="w-56 bg-neutral-900 p-2 border-l border-neutral-800 flex flex-col gap-2 overflow-y-auto">
      ...
      <div className="text-center">
        <ContextMenu>
          <ContextMenuTrigger className="w-full relative group" aria-label="Grimório">
            <button onClick={() => actions.drawCard()} className="w-full appearance-none">
              <p className="text-sm font-medium text-amber-400 flex items-center justify-center gap-2 mb-1">
                <Library size={16} /> Grimório ({library.length})
              </p>
              <Image
                src={cardBackImg}
                alt="Verso da carta"
                ...
                height={200}
                unoptimized
                className="rounded-lg shadow-lg mx-auto transition-transform duration-200 group-hover:scale-105"
              />
            </button>
          </ContextMenuTrigger>
          <ContextMenuContent className="w-56 bg-neutral-900 border-neutral-700 text-neutral-200">
            ...
            <ContextMenuItem
              className="cursor-pointer"
              onSelect={() => onViewZone('Topo do Grimório (5)', library.slice(0, 5))}
            >
              <Eye className="mr-2 h-4 w-4" /> Ver as 5 do Topo
            </ContextMenuItem>
            <ContextMenuSub>
              <ContextMenuSubTrigger>
                <ArrowDownToLine className="mr-2 h-4 w-4" /> Millar Cartas
              </ContextMenuSubTrigger>
              <ContextMenuSubContent className="bg-neutral-900 border-neutral-700 text-neutral-200">
                <ContextMenuItem onSelect={() => actions.millCards(1)} className="cursor-pointer">
                  Millar 1
                </ContextMenuItem>
                <ContextMenuItem onSelect={() => actions.millCards(3)} className="cursor-pointer">
                  Millar 3
                </ContextMenuItem>
                <ContextMenuItem onSelect={() => actions.millCards(5)} className="cursor-pointer">
                  Millar 5
                </ContextMenuItem>
              </ContextMenuSubContent>
            </ContextMenuSub>
      ...
      <div>
        <CardContent className="p-3 flex flex-col gap-2">
          ...
          <Button variant="destructive" onClick={() => actions.resetGame()}>
