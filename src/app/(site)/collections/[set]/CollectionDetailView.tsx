...
export default function CollectionDetailView({ setInfo, initialCards, setCode }: CollectionDetailViewProps) {
    ...
    const buildSearchUrl = useCallback((pageUrl?: string) => {
    ...
    const fetchCards = useCallback(async (isNewSearch = false) => {
        if (loading) return;
        setLoading(true);
        ...
        <div className="min-h-screen bg-neutral-950 text-neutral-100 flex flex-col">
            ...
            <header className="relative bg-neutral-900 border-b border-neutral-800 pb-8 pt-8 px-4 overflow-hidden">
                ...
                <div className="container mx-auto max-w-7xl relative z-10">
                    <Link href="/collections" className="inline-flex items-center text-neutral-400 hover:text-amber-500 mb-6 transition-colors">
                    ...
                    <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
                        ...
                        <div className="flex-1 text-center md:text-left space-y-2">
                            <h1 className="text-3xl md:text-5xl font-bold text-white tracking-tight">{setInfo.name}</h1>
            ...
            <div className="container mx-auto max-w-7xl p-4 md:p-8 space-y-8">
                ...
                <div className="bg-neutral-900/80 backdrop-blur-md border border-neutral-800 p-4 md:p-6 rounded-2xl flex flex-col xl:flex-row gap-4 sticky top-4 z-30 shadow-2xl shadow-black/50 transition-all duration-300">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full xl:w-auto">
                        ...
                        {(searchQuery || rarityFilter !== 'all' || colorFilter !== 'all' || typeFilter !== 'all') && (
                            <Button
                                variant="ghost"
                                onClick={() => {
                                    setSearchQuery('');
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-6">
                    {cards.map((card, index) => {
                        ...
                        return (
                            <div ref={isLast ? lastCardRef : null} key={card.id} className="transform transition-transform duration-300 hover:scale-[1.03] hover:z-10">
                                <CardItem
                                    card={card}
                                />
