...
export async function generateDeckContentPackage(
  ...
  const cardCount =
    (contentPackage.decklist.mainboard.reduce((s: any, c: any) => s + c.count, 0) || 0) +
    (contentPackage.decklist.sideboard?.reduce((s: any, c: any) => s + c.count, 0) || 0);
...
export async function deleteAiGeneration(generationId: string, savedDeckId: string | null) {
  ...
  if (savedDeckId) {
    ...
    const { error: deckError } = await supabase.from('decks').delete().eq('id', savedDeckId);
